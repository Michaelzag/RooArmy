...
function assembleQuestions(componentsDir, outputFile) {
  ...
  try {
    ...
    // Load phase3 team focus
    const phase3File = path.join(componentsDir, 'phase3_team_focus.json');
    if (fs.existsSync(phase3File)) {
      const phase3Data = loadJSON(phase3File);
      if (phase3Data) {
        if (phase3Data.team) {
          questions.assessment.phase3_roles_focus = phase3Data.team;
        }
        if (phase3Data.solo) {
          questions.assessment.phase3_solo_focus = phase3Data.solo;
        }
      }
    }
...
async function setupTest() {
  try {
    ...
    console.log('You can now open roo-army-test/ in a new window and test the RooCommander 3.5.');
